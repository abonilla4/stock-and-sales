-- ============================================================
-- Migración 00011: Origen de autorización en ventas
-- 1. Agrega la columna origen_autorizacion ('admin_online' | 'offline_diferido') a ventas.
-- 2. Actualiza procesar_venta_transaccion para manejar p_origen_autorizacion.
--    - Si origen = 'offline_diferido': autorizado_por queda NULL, autorizado_en registra la hora de sync.
--    - Si origen = 'admin_online': exige p_autorizado_por NOT NULL.
-- ============================================================

-- 1. Agregar columna origen_autorizacion a ventas
ALTER TABLE public.ventas
  ADD COLUMN IF NOT EXISTS origen_autorizacion text;

-- 2. Recrear procesar_venta_transaccion
...
BEGIN
  ...
  -- Lógica de Auditoría según origen
  IF p_permitir_stock_negativo THEN
    IF p_origen_autorizacion = 'offline_diferido' THEN
      v_autorizado_por := NULL; -- Nadie autorizó en vivo; requiere revisión posterior
      v_autorizado_en := now();
      v_origen_autorizacion := 'offline_diferido';
    ELSE
      -- admin_online por defecto
