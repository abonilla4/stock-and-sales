-- ============================================================
-- Migración 00010: Auditoría de autorización en ventas
-- Agrega autorizado_por (FK → auth.users) y autorizado_en a la tabla ventas,
-- y actualiza procesar_venta_transaccion para registrar quién autorizó
-- una venta con stock insuficiente DENTRO de la transacción atómica.
-- ============================================================
...
-- 2. Recrear la función con soporte para autorizado_por/autorizado_en
...
BEGIN
  -- ================================================================
  -- 0. Idempotencia: si ya existe una venta con este client_tx_id,
  --    retornar sus datos sin duplicar.
  -- ================================================================
  ...
  -- Si se autoriza stock negativo, debe haber un autorizador
  IF p_permitir_stock_negativo THEN
    IF p_autorizado_por IS NULL THEN
      RAISE EXCEPTION 'Se requiere autorizado_por cuando se permite stock negativo.';
    END IF;
    v_autorizado_en := now();
  END IF;

  -- ================================================================
  -- 1. Bloqueo de filas y verificación de stock para cada producto
  -- ================================================================
  ...
  -- ================================================================
  -- 2. Si es venta fiada, bloquear la fila del cliente para saldo_fiado
  -- ================================================================
  ...
  -- ================================================================
  -- 3. Insertar cabecera de la venta (snapshot congelado + auditoría)
  -- ================================================================
  INSERT INTO public.ventas (
    ...
    true,
    p_client_tx_id,
    CASE WHEN p_permitir_stock_negativo THEN p_autorizado_por ELSE NULL END,
    v_autorizado_en
  )
  RETURNING id INTO v_venta_id;

  -- ================================================================
  -- 4. Procesar líneas: detalle, descuento de stock, movimientos
  -- ================================================================
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    v_producto_id     := (v_item->>'producto_id')::uuid;
    ...
    -- Insertar línea de detalle (precio congelado)
    ...
    INSERT INTO public.movimientos_inventario (
      ...
      v_cantidad,
      CASE
        WHEN p_permitir_stock_negativo THEN 'Venta en POS (autorizado por admin: stock insuficiente)'
        ELSE 'Venta en POS'
      END,
      v_venta_id
    );
  ...
  -- ================================================================
  -- 5. Si es fiado, incrementar saldo_fiado atómicamente
  -- ================================================================
  IF p_metodo_pago = 'fiado' AND p_cliente_id IS NOT NULL THEN
  ...
  -- ================================================================
  -- 6. Retornar resultado con datos confirmados para el recibo
  -- ================================================================
