...
export function ClienteDetalleClient({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header Info Cliente */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl border bg-card p-6 shadow-sm">
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Cédula / RIF: <strong className="text-foreground">{cliente.identificacion ?? "No registrado"}</strong> • Teléfono: <strong className="text-foreground">{cliente.telefono ?? "No registrado"}</strong>
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => setEditOpen(true)}>
            <Edit className="mr-1.5 size-4" /> Editar Datos
          </Button>
          <Button size="sm" onClick={() => setAbonoOpen(true)} className="bg-emerald-600 hover:bg-emerald-700 text-white">
            <DollarSign className="mr-1.5 size-4" /> Registrar Abono
          </Button>
        </div>
      </div>

      {/* Tarjeta de Saldo a Crédito y Resumen */}
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <Card className={tieneDeuda ? "border-amber-300 bg-amber-50/40 dark:border-amber-800 dark:bg-amber-950/20" : ""}>
          ...
          <CardContent>
            <p className="font-mono text-2xl font-extrabold text-amber-700 dark:text-amber-400">
              {formatUSD(saldoUsd)} <span className="text-xs font-semibold text-muted-foreground">USD</span>
            </p>
            <p className="mt-1 text-xs text-muted-foreground font-mono">
              Equivalente: {formatBs(saldoBs)}
            </p>
          </CardContent>
        </Card>
